...
- Book a consultation, pay through SSLCommerz (bKash, cards, banks), and join the video
  call from the same page. Big buttons and short lines in Bangla and English.

**Doctors**

- Switch between online and offline with one control in the header.
- See today's consultations, start the call, write notes, and keep a profile.

**Admin**

- Doctors, patients and consultations in one place, with counts that come from the
  database.
...
| API | Laravel 13, PHP 8.4, Sanctum, session cookies |
...
Video: the API checks that the signed in user belongs to the consultation and that it is
paid, then builds an Agora token for a numeric uid with a two hour limit. The React call
...
Add `CORS_ALLOWED_ORIGINS=http://127.0.0.1:3001` to the API `.env`. Open
http://127.0.0.1:3001. The seed creates three doctors, a patient and one paid
...
frontend/  Next.js app: src/app (routes), src/components (brand, site, app shell, video call)
...
and the deployment moved to container images.
